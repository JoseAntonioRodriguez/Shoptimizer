...
class Eroski(Shop):
    ...
    def get_product_list_page(self):
        ...
        resp = session.post("https://www.compraonline.grupoeroski.com/ecoventa/actions/accesoUsuarioRegistrado.do", data = form_params)
        self.log(resp)
        ...
        form_params = {"ultimasCompras": "NO", 
                       "sarfnbr": "3251371"}
        resp = session.post("https://www.compraonline.grupoeroski.com/ecoventa/actions/seleccionarDireccionEnvio.do", data = form_params, allow_redirects = False)
        self.log(resp)
        
        resp = session.get("http://www.compraonline.grupoeroski.com/ecoventa/actions/mostrarListaHabitual.do?idLista=" + self.idLista) #1
        self.log(resp)
    ...
    def parse_product_list_page(self, html_page):
        ...
        product_list = html_tree.xpath("//table[@id='conte']/form/tr[starts-with(@id, 'prod_') or starts-with(@id, 'categ_')]")
        ...
        for product_item in product_list:
            ...
            if len(product_item.xpath(base_xpath + "/tr[3]/td/table/tr/td[@class='sub_menu_11']/a/strong[text()='Busca Sustituto']")) > 0:
                continue
            
            product_id = product_id.partition("_")[2]
            product_name = product_item.xpath(base_xpath + "/tr/td/table/tr/td[@class='menu_sup11']")[0].text_content().strip()
            product_price = float(product_item.xpath(base_xpath + "/tr[3]/td/table/tr/td[@class='menu_12_rojo_sin']/strong")[0].text.partition(" ")[0].replace(",", "."))
