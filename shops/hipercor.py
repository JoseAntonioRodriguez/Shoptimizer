...
class Hipercor(Shop):
    ...
    def get_product_list_page(self):
        ...
        resp = session.get("https://www.hipercor.es/profile2/profile/auth/TAM/AutenticaUsuario?migrar=1&datincom=0&urltam=https://www.hipercor.es/hipercor/sm2/login/login.jsp")
        self.log(resp)
        ...
        form_params = {"_dyncharset": "iso-8859-15", 
                       ...
                       "group1": "homeDelivery"}
        resp = session.post("https://www.hipercor.es/profile2/profile/LoginServlet?_DARGS=/sm2/common/public/homeHipercorContainer.jsp", data = form_params)
        self.log(resp)
        ...
        form_params = {"login-form-type": "pwd", 
                       "username": username, 
                       "password": password}
        resp = session.post("https://www.hipercor.es/pkmslogin.form?pag_error=https://www.hipercor.es/hipercor/sm2/login/login.jsp?_errorValidationTAM=true", data = form_params)
        self.log(resp)
        #resp = session.get("https://www.hipercor.es/profile2/profile/auth/TAM/AutenticaUsuario?migrar=1&datincom=0&urltam=https://www.hipercor.es/hipercor/sm2/login/login.jsp", allow_redirects = False)
        ...
        resp = session.post("https://www.hipercor.es/hipercor/sm2/login/login.jsp?_DARGS=/sm2/login/LoginOptionsLoggedUser.jsp.frmlogin", data = form_params)
        self.log(resp)
        #resp = session.get(resp.headers["Location"], allow_redirects = False)
        #resp = session.get(resp.headers["Location"], allow_redirects = False)
        #resp = session.get(resp.headers["Location"], allow_redirects = False)
        ...
        resp = session.get("http://www.hipercor.es/hipercor/sm2/wishlist/wishList.jsp?giftListId=" + self.giftListId) #gl126585719
        self.log(resp)
    ...
    def parse_product_list_page(self, html_page):
        ...
        for product_item in product_list:
            ...
            product_price = float(product_item.xpath(".//p[@class='ahora']/span")[0].text.strip().partition(" ")[0].replace(",", "."))
            
            product_unitary_price = product_item.xpath(".//div[contains(concat(' ', normalize-space(@class), ' '), ' precio_kg ')]")
            ...
            Shop.add_product(self, Product(product_id, product_name, product_price, product_unitary_price, product_unit))
